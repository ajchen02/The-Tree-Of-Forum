var systemComponents = {
	...
	'overlay-head': {
		...
		<br>
		<span v-if="canGenPoints()"  class="overlayThing">({{tmp.other.oompsMag != 0 ? format(tmp.other.oomps) + " OOM" + (tmp.other.oompsMag < 0 ? "^OOM" : tmp.other.oompsMag > 1 ? "^" + tmp.other.oompsMag : "") + "s" : formatSmall(getPointGen().total)}}/sec)</span>
    ...
    'info-tab': {
        ...
        <h2>{{modInfo.name}}</h2>
        <br>
        <h3>{{VERSION.withoutName}}: <del>{{VERSION.name}}</h3></del>
